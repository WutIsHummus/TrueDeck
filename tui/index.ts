...
async function main(): Promise<void> {
 ...
 // smartCSR leaves half-erased lines when preview content shrinks between
 // ticks ("old text cut off until new output arrives"). Full CSR is safer.
 smartCSR: false,
 fullUnicode: true,
 title: 'TrueDeck',
 mouse: true,
 ...
 height: '100%-6',
 border: { type: 'line' },
 // tags only for our chrome strings - raw PTY tail is escaped separately
 tags: true,
 scrollable: true,
 alwaysScroll: true,
 mouse: true,
 ...
 /** Strip ANSI + escape blessed tag braces so agent output cannot "eat" preview text. */
 function safePreviewText(raw: string): string {
 // Drop CSI / OSC / other ESC sequences (preview is plain text, not a VT emulator)
 let t = raw
 ...
 .replace(/\r\n/g, '\n')
 .replace(/\r/g, '\n')
 // Blessed tags: {word} - neutralize braces from agent output
 t = t.replace(/\{/g, '{open}').replace(/\}/g, '{close}')
 return t
 }
 ...
 // Prefer a clean line start so we don't open mid-CSI from the ring buffer
 let slice = s.buf.slice(-6000)
 const nl = slice.indexOf('\n')
 if (nl > 0 && nl < 200) slice = slice.slice(nl + 1)
 const tail = safePreviewText(slice)
 preview.setContent(
 `{bold}${s.name}{/} {gray-fg}${shortPath(s.cwd)}{/} {red-fg}[w = close this tab]{/}\n\n${tail}`
 )
 ...
 `\x1b[36mTrueDeck attach: ${s.name} · Ctrl+] detach · Ctrl+W close\x1b[0m\r\n`
 )
 ...
 // Flush recent buffer from a clean ESC/line boundary (avoid mid-CSI garbage)
 if (s.buf) {
 let dump = s.buf.slice(-8000)
 const esc = dump.lastIndexOf('\x1b[', Math.min(200, dump.length))
 const nl = dump.indexOf('\n')
 if (esc > 0 && esc < 80) dump = dump.slice(esc)
 else if (nl > 0 && nl < 200) dump = dump.slice(nl + 1)
 ...
 // node-pty IEvent returns IDisposable - must dispose on detach or output
 // keeps writing to stdout while blessed owns the screen (cut/garbled deck).
 const dataSub = s.proc.onData(onData)
 ...
 dataSub: { dispose: () => void } | ((data: string) => void),
 onStdin: (chunk: Buffer) => void,
 closeTab = false
 ): void {
 process.stdin.off('data', onStdin)
 try {
 if (dataSub && typeof dataSub === 'object' && 'dispose' in dataSub) {
 dataSub.dispose()
 }
 } catch {
 // ignore
 }
 const extra = s as unknown as { _detachResize?: () => void }
 try {
 extra._detachResize?.()
 } catch {
 // ignore
 ...
 screen.enter()
 list.focus()
 // Full invalidate so leftover attach-mode glyphs don't stick
 try {
 screen.realloc()
 } catch {
 // ignore
